...
def prep_image_cpu(img, inp_dim):
    """
    ...
    #print("in function preprocess image cpu")
    #print("img.shape");print(img.shape)
    orig_im = img
    #print("orig_im.shape");print(orig_im.shape)
    dim = orig_im.shape[1], orig_im.shape[0]
    img = (letterbox_image(orig_im, (inp_dim, inp_dim)))
    img_ = img[:,:,::-1].transpose((2,0,1)).copy()
    img_ = torch.from_numpy(img_).float().div(255.0).unsqueeze(0)
    #img_ = torch.from_numpy(img).float().div(255.0).unsqueeze(0)
    #print("img_.shape");print(img_.shape)
    #print("orig_im.shape");print(orig_im.shape)
    #print("dim.shape");print(dim)
    #print("return prep image cpu")
...
def prep_image_pil(img, network_dim):
    ...
    print('network_dim dim {0}'.format(network_dim.shape))
    print('network_dim type {0}'.format(type(network_dim)))
