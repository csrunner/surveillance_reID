...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    for ind in range(batch_size):
        ...
        person_mask = (image_pred_[:,-1] < 0.1).float().unsqueeze(1)
        image_pred_ = image_pred_ * person_mask
        #print("(1)after person mask: image_pred_")
        #print(image_pred_)

        person_ind = (torch.nonzero(image_pred_[:,4]))
        image_pred_ = image_pred_[person_ind.squeeze(),:].view(-1,7)
        #print("(2)get rid of zerolines: image_pred_")
        #print(image_pred_)
        if len(image_pred_) == 0:
            #print("zeorlines,continue")
            continue
    ...
    #print("write result return shape")
    #print(output.shape)
    #print(output)
    if not write:
        return 0
